...
def encontrar_moneda_falsa_recursivo(M: list, inicio: int, fin: int) -> tuple[int, int]:
    """
    Función recursiva Top-Down que halla el índice de la moneda falsa
    y cuenta cuántas pesadas se realizan.
    """
    ...
    if suma_A < suma_B:
        indice, pesadas = encontrar_moneda_falsa_recursivo(M, inicio, fin_A)
    elif suma_B < suma_A:
        indice, pesadas = encontrar_moneda_falsa_recursivo(M, inicio_B, fin_B)
    else:
        # La moneda defectuosa está en el grupo C (resto del arreglo)
        inicio_C = inicio + 2 * k
        indice, pesadas = encontrar_moneda_falsa_recursivo(M, inicio_C, fin)

    return indice, pesadas + 1


def buscar_moneda_falsa(M: list) -> tuple[int, int]:
    """
    Función principal que inicializa la búsqueda.
    Retorna una tupla (índice_moneda_falsa, pesadas_realizadas).
    """
    if not M:
...
# Para ejecutar:
# python3 problema_1_pesada.py test_data/100.txt (u otro txt con el mismo formato)
...
if __name__ == "__main__":
    ...
    solucion_obtenida, pesadas = buscar_moneda_falsa(monedas)
    ...
    print("Pesadas realizadas: " + str(pesadas))
